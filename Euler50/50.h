...
std::vector<unsigned int>       primesb;
...
void morePrimes(unsigned int num)
{
    ...
    for (auto i = primesb.back() + 2; primesb.size() <= num; i += 2)
    {
        bool isPrime = true;
    ...
    for (auto i = primeSum.size(); i < primesb.size(); i++)
        primeSum.push_back(primeSum.back() + primesb[i]);
}
...
int mainBrumme()
{
    ...
    while (tests--)
    {
        ...
        while (primesb[start] <= 131 && primesb[start] <= last)
        {
            ...
            while (primeSum[pos] - subtract <= last)
            {
                ...
                if (pos + 100 >= primesb.size()) // plus 100 is probably too cautious
                    morePrimes(primesb.size() + PrimesPerBatch);
            }
            pos--;
...
std::pair<unsigned long long, unsigned int> get_solution_brumme(unsigned long long last)
{
    // generate some primes
    const unsigned int PrimesPerBatch = 10000;
    morePrimes(PrimesPerBatch);
    ...
    if (best >= 2)
        maxLength++;
    //std::cout << best << " " << maxLength << std::endl;
    return std::pair<unsigned long long, unsigned int>(best, maxLength);
}
